 ...
 for(i=0;i<=8;i++){
  ...
  for(i=0;i<=8;i++){
      ...
      if(e.getSource()==reset){
       ...
       for(i=0;i<=7;i++)
        for(j=0;j<=4;j++)
		a[i][j]=a1[i][j];   // khởi tạo lại mảng
        }
        else{  //complogic
            for(i=0;i<=8;i++){
               if(e.getSource()==b[i]){
...
for(i=0;i<=7;i++){
  ...
  Icon icon3=b[(a[i][3]-1)].getIcon();
	...
	JOptionPane.showMessageDialog(TTT1.this,"!!!Bạn thắng!!! click reset");			 break;
                   }
             else if(icon1==ic2){ 
             b[(a[i][1]-1)].setIcon(ic22);
             b[(a[i][2]-1)].setIcon(ic22);
             b[(a[i][3]-1)].setIcon(ic22); 
               JOptionPane.showMessageDialog(TTT1.this,"!!!AWK (COMPUTER) Máy chiến thắng!!! click reset");
                break;			 
               }
         }
    }  
